...
sh ''' for i in `echo ${chat_id} | sed "s/,/  /g"` ; do
   ...
   curl  -s --max-time 10 -F chat_id=${i} -F document=@result.txt  -F caption="${buildStatus}: Job ${env.JOB_NAME} [${env.BUILD_NUMBER}]"  https://api.telegram.org/bot${token}/sendDocument 
	done '''
