...
////    
    ...
    stage('Loading configs...') {
      ...
      writeFile file: 'roles/deploy/tasks/custom/pre_symlink.sh', text: ('set -o pipefail; ' +  'set -ex; ' + ' cd $1 ; ' + deploy_pre_symlink)
      writeFile file: 'roles/deploy/tasks/custom/after_symlink.sh', text: ('set -o pipefail; ' +  'set -ex; ' + ' cd $1 ; ' + deploy_after_symlink)
      writeFile file: 'roles/rollback/tasks/custom/pre_symlink.sh', text: ('set -o pipefail; ' +  'set -ex; ' + ' cd $1 ; ' + rollback_pre_symlink)
      writeFile file: 'roles/rollback/tasks/custom/after_symlink.sh', text: ('set -o pipefail; ' +  'set -ex; ' + ' cd $1 ; ' + rollback_after_symlink)
    
      writeFile file: 'roles/deploy/tasks/custom/pre_symlink_once.sh', text: ('set -o pipefail; ' +  'set -ex; ' + ' cd $1 ; ' + deploy_pre_symlink_once)
      writeFile file: 'roles/deploy/tasks/custom/after_symlink_once.sh', text: ('set -o pipefail; ' +  'set -ex; ' + ' cd $1 ; ' + deploy_after_symlink_once)
      writeFile file: 'roles/rollback/tasks/custom/pre_symlink_once.sh', text: ('set -o pipefail; ' +  'set -ex; ' + ' cd $1 ; ' + rollback_pre_symlink_once)
      writeFile file: 'roles/rollback/tasks/custom/after_symlink_once.sh', text: ('set -o pipefail; ' +  'set -ex; ' + ' cd $1 ; ' + rollback_after_symlink_once)
